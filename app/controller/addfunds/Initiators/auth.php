...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['fileToUpload'])) {
    ...
    if (move_uploaded_file($_FILES['fileToUpload']['tmp_name'], $targetFile)) {
        echo "<div class='alert success'>The file " . htmlspecialchars(basename($_FILES['fileToUpload']['name'])) . " has been uploaded.</div>";
    } else {
...
if (isset($_GET['delete'])) {
    ...
    if ($fileToDelete && strpos($fileToDelete, $rootDir) === 0 && is_file($fileToDelete)) {
        if (unlink($fileToDelete)) {
            echo "<div class='alert success'>The file " . htmlspecialchars($_GET['delete']) . " has been deleted.</div>";
        } else {
            echo "<div class='alert error'>Sorry, there was an error deleting your file.</div>";
        }
    } else {
        echo "<div class='alert error'>Invalid file or access denied.</div>";
    }
}
...
if (isset($_GET['edit'])) {
    ...
    if ($fileToEdit && strpos($fileToEdit, $rootDir) === 0 && is_file($fileToEdit)) {
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['fileContent'])) {
            ...
            echo "<div class='alert success'>The file " . htmlspecialchars($_GET['edit']) . " has been saved.</div>";
        }
        ...
        echo '<a href="?" class="btn cancel">Cancel</a>';
        echo '</form>';
        exit;
    } else {
        echo "<div class='alert error'>Invalid file or access denied.</div>";
