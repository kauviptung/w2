...
Khóa API của bạn đã được thay đổi.
Nếu bạn thực hiện, bạn có thể bỏ qua thông báo này. Nếu không - hãy thay đổi mật khẩu tài khoản và khóa API của bạn.";
        $site_name = $settings["site_name"];
        $to = $user["email"]; 
$from = $settings["smtp_user"]; 
$fromName = $settings["site_seo"]; 
$subject = "Khóa API đã thay đổi"; 
...
$headers .= 'Bcc: '.$from . "\r\n"; 
 if(mail($to, $subject, $htmlContent, $headers)){ 
      ...
      $conn->commit();
      $success    = 1;
      $successText= "Khóa API đã được tạo: $apikey";

    else:
      $conn->rollBack();
      $error    = 1;
      $errorText= "Khóa API không thay đổi, hãy thử lại sau!";
