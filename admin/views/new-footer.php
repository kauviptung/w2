...
<script>
    ...
    user_selector = new TomSelect('#select-user', {
      ...
      render: {
        loading: function (data, escape) {
          return '<center><svg class="spinner small" viewBox="0 0 48 48"><circle class="path" cx="24" cy="24" r="20" fill="none" stroke-width="3"></circle></svg> Đang tải người dùng...</center>';
        },
    ...
    category_selector = new TomSelect('#select-category', {
      ...
      render: {
        loading: function (data, escape) {
          ...
          return '<center><svg class="spinner small" viewBox="0 0 48 48"><circle class="path" cx="24" cy="24" r="20" fill="none" stroke-width="3"></circle></svg> Đang tải danh mục...</center>';
    ...
    service_selector = new TomSelect('#select-service', {
      ...
      render: {
        loading: function (data, escape) {
          ...
          return '<center><svg class="spinner small" viewBox="0 0 48 48"><circle class="path" cx="24" cy="24" r="20" fill="none" stroke-width="3"></circle></svg> Đang tải dịch vụ...</center>';
  ...
  function show_table_loader() {
    ...
    $(".table tbody").html('<tr><td colspan="6"><center><svg class="spinner medium" viewBox="0 0 48 48"><circle class="path" cx="24" cy="24" r="20" fill="none" stroke-width="5"></circle></svg> Đang tải...</center></td></tr>');
  }

  function no_data_table() {
    $(".table").removeClass("loading");
    $(".table").parent().removeClass("table-responsive");
    $(".table tbody").html('<tr><td colspan="6"><center><b>Không có dữ liệu</b></center></td></tr>');
  }
  ...
  $(document).on("click", "[data-bs-toggle='modal']", function () {
    ...
    if (form == "create_special_price") {
      ...
      var body = '<form method="POST" action="admin/special-pricing/create-new"><div class="mb-3"><label class="form-label">Thành viên</label><select id="select-user" name="special_price_user" class="fsb-ignore" placeholder="Nhập để tìm kiếm..."></select></div><div class="mb-3"><label class="form-label">Danh mục</label><select id="select-category" class="fsb-ignore" placeholder="Nhập để tìm kiếm..."></select></div><div style="display:none;" class="mb-3"><label class="form-label">Dịch vụ</label><select id="select-service" class="fsb-ignore" name="special_price_service" placeholder="Nhập để tìm kiếm..."></select></div><div id="special_pricing_service_info" style="display:none;" class="row"><div class="col"><label class="form-label">Giá/1000</label><div class="input-group mb-3"><span class="input-group-text">' + panel_settings.site_currency_symbol + '</span><input type="number" id="special_pricing_selected_service_price" class="form-control" disabled readonly></div></div><div class="col"><label class="form-label">Chi phí cho 1000</label><div class="input-group mb-3"><span class="input-group-text">' + panel_settings.site_currency_symbol + '</span><input id="special_pricing_selected_service_cost" type="number" class="form-control" disabled readonly></div></div><div class="form-group"><label class="form-label">Giá đặc biệt cho 1000</label><div class="input-group mb-3"><span class="input-group-text">' + panel_settings.site_currency_symbol + '</span><input type="number" step="0.01" name="special_price_for_service" id="special_price_for_service"  class="form-control" placeholder="Nhập giá đặc biệt cho dịch vụ..."></div></div></div><div class="custom-modal-footer"><button type="button" class="btn btn-danger" data-bs-dismiss="modal">Hủy</button>&nbsp;&nbsp;<button type="submit" data-loading-text="Creating..." class="btn btn-primary">Tạo</button></div></form>';
        ...
        success: function (json) {
          ...
          if (data.length > 0) {
            for (i = 0; i < data.length; i++) {
              ...
              tbody += '<tr><td>'+data[i].id+'</td><td><span class="badge bg-secondary">'+data[i].cid+'</span> '+data[i].username+'</td><td>'+data[i].method+' '+mode+'</td><td>'+panel_settings.site_currency_symbol+" "+data[i].user_balance+'</td><td>'+panel_settings.site_currency_symbol+" "+data[i].amount+'</td><td>'+data[i].status+'</td><td>'+data[i].created_at+'</td><td><div class="btn-group dropstart"><button type="button" class="btn btn-secondary dropdown-toggle btn-sm" data-bs-toggle="dropdown" aria-expanded="false">Hành động</button><ul class="dropdown-menu"><li><a class="dropdown-item">Chi tiết</a></li></ul></div></td></tr>';
            }
            ready_table();
            $(".table tbody").html(tbody);
          } else {
