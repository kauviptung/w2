  ...
  if ($api["api_type"] == 3):
    echo '<div class="service-mode__block">
          <div class="form-group">
            <label>Service</label>
            <input class="form-control" name="service" placeholder="Nhập ID dịch vụ">
          </div>
        </div>';
...
elseif ($action == "pay_rent"):
  if (!$_POST["state"]) {
    ...
    if (!$_SESSION["upi"]) {
      $upi = "upi://pay?pa=saruk02@ybl&pn=Gurmeet%20Singh&am=$amount&mam=$amount&tn=Monthly%20Rent%20Payment&tr=$txn_id";
      $_SESSION["upi"] = $upi;
    }
    ...
    $content .= '<br>
  <center>
  <p>Quét mã QR được tạo bên dưới và đợi giao dịch được xác minh.</p>
...
';
    ...
    if ($resp["STATUS"] == "TXN_SUCCESS") {
      ...
      $htmlContent = $_SERVER["HTTP_HOST"] . " Đã trả tiền thuê nhà <br><pre>" . json_encode($resp) . "</pre>";
...
elseif ($action == "bulkGetCategories"):
  ...
  $resp .= '<div class="form-group"><button type="button" class="btn btn-primary" id="selectCategory">Lựa chọn</button></div>';
...
<br>
<center><p style="font-size:19px;">Tổng số dịch vụ : <b>' . $total . '</b></p></center>
<div class="form-group">
<label>URL trang dịch vụ của nhà cung cấp</label>
<input type="text" class="form-control" value="https://' . GET_API_NAME_BY_ID($_POST["id"]) . '/services" name="service_page_url">
</div>';


$content .= '<div class="form-group"><label>Ngôn ngữ</label><select class="form-control" name="language">';
...
<div class="form-group">
<button type="submit" class="btn btn-success">Mô tả chụp</button> </div>
</form>
</div></div></div>
<script>
...
$(".capture_desc_services").selectpicker();
 ...
 Không tìm thấy dịch vụ nào được nhập khẩu với người bán này.
</div></div>'
    );
  }
  echo json_encode($array, true);
...
dripfeed_interval=:interval, dripfeed_totalquantity=:totalquantity, dripfeed_delivery=:delivery");
    ...
    $array = array(
      "success" => true,
      "message" => 'Một đơn hàng giả đã được đặt với ID đơn hàng : ' . $next_order_id
    );
  } else {
    $array = array(
      ...
      "message" => "ID ĐƠN HÀNG Bạn nhập không hợp lệ."
    );
  }
...
elseif ($action == "set_discount_percentage"):
  ...
  echo json_encode(["content" => $return, "title" => "Đặt phần trăm giảm giá <span class=\"badge badge-primary\">" . $row["username"] . "</span>"]);
...
if (count($languages) > 1):
    $translationList = '<a class="other_services"> Bản dịch (' . (count($languages) - 1) . ') </a>';
  else:
    $translationList = '';
  endif;
  foreach ($languages as $language):
...
<input type="text" class="form-control" name="menu_name[' . $language["language_code"] . ']">
  ...
  if (count($languages) > 1):
    $return .= '</div>';
     ...
     $return .=     '
<input type="hidden" class="form-control" name="visible" value="Nội bộ">
         ...
         <input type="text" class="form-control" name="slug" value="" placeholder="/page">
          ...
          <div class="form-group">
            <label class="form-group__service-name">Biểu tượng Menu</label>
...
e.iconpickerInstance.options.fullClassFormatter(e.iconpickerValue);
  ...
  echo json_encode(["content" => $return, "title" => "Thêm Menu"]);
 ...
 if (count($languages) > 1):
  ...
  foreach ($languages as $language):
    if ($language["default_language"]):
      $return .= '
        <div class="form-group">
<label class="form-group__service-name">Tên Menu <span class="badge">' . $language["language_name"] . '</span> ' . $translationList . ' </label>
<input type="text" class="form-control" name="menu_name[' . $language["language_code"] . ']" value="' . $multiName[$language["language_code"]] . '">
      ...
      $return .= '<div class="form-group">
<label class="form-group__service-name">Tên Menu <span class="badge">' . $language["language_name"] . '</span> </label>
<input type="text" class="form-control" name="menu_name[' . $language["language_code"] . ']" value="' . $multiName[$language["language_code"]] . '">
                </div>';
    endif;
  endforeach;
 ...
 if (count($languages) > 1):
    ...
    foreach ($languages as $language):
      if ($language["default_language"]):
        $return .= '
          <div class="form-group">
<label class="form-group__service-name">Tiêu đề thông báo <span class="badge">' . $language["language_name"] . '</span> ' . $translationList . ' </label>
<input type="text" class="form-control" name="announcement_title[' . $language["language_code"] . ']" >
        ...
        $return .= '<div class="form-group">
<label class="form-group__service-name">Tiêu đề thông báo <span class="badge">' . $language["language_name"] . '</span> </label>
<input type="text" class="form-control" name="announcement_title[' . $language["language_code"] . ']" >
                  </div>';
      endif;
    endforeach;
...
if (count($languages) > 1):
    ...
    foreach ($languages as $language):
      if ($language["default_language"]):
        $return .= '
<div class="form-group">
<label class="form-group__service-name">Nội dung thông báo <span class="badge">' . $language["language_name"] . '</span> ' . $translationList . ' </label> <textarea class="form-control summernote-editor" name="announcement_content[' . $language["language_code"] . ']"></textarea></div>';
        if (count($languages) > 1):
        ...
        $return .= '
<div class="form-group">
<label class="form-group__service-name">Nội dung thông báo <span class="badge">' . $language["language_name"] . '</span> </label> <textarea class="form-control summernote-editor" name="announcement_content[' . $language["language_code"] . ']"></textarea></div>';
      endif;
    endforeach;
    if (count($languages) > 1):
      $return .= '</div>';
...
$(\".other_services\").click(function(){
 ...
 $(\".other_services1\").click(function(){
  ...
  echo json_encode(["content" => $return, "title" => "Thêm thông báo mới"]);
...
if (count($languages) > 1):
    ...
    foreach ($languages as $language):
      if ($language["default_language"]):
        $return .= '
<div class="form-group">
<label class="form-group__service-name">Nội dung thông báo <span class="badge">' . $language["language_name"] . '</span> ' . $translationList . ' </label> <textarea class="form-control summernote-editor" name="announcement_content[' . $language["language_code"] . ']">'.$news_content_lang[$language["language_code"]].'</textarea></div>';
        if (count($languages) > 1):
          $return .= '<div class="hidden" id="translationsList1">';
        endif;
      else:
        $return .= '
<div class="form-group">
<label class="form-group__service-name">Nội dung thông báo <span class="badge">' . $language["language_name"] . '</span> </label> <textarea class="form-control summernote-editor" name="announcement_content[' . $language["language_code"] . ']">'.$news_content_lang[$language["language_code"]].'</textarea></div>';
      endif;
    endforeach;
    if (count($languages) > 1):
      $return .= '</div>';
...
$(\".other_services\").click(function(){
 ...
 $(\".other_services1\").click(function(){
  ...
  echo json_encode(["content" => $return, "title" => "Chỉnh sửa thông báo "]);
...
if( result == 1 ){
  ...
  $.toast({
      ...
      text: "Hoạt động không thành công",
      icon: "error",
      loader: true,
      loaderBg: "#9EC600"
  });
          ...
          <div class="form-group">
            ...
            <div class="input-group">
              ...
              <span class="input-group-btn">
                ...
                <span class="fa fa-random" data-toggle="tooltip" data-placement="bottom" title="" aria-hidden="true" data-original-title="Tạo mật khẩu"></span></button>
              </span>
            </div>
          </div>
...
<option value="1">Có</option>
          ...
          <div class="form-group" id="debit_limit">
            <label>Số dư âm là bao nhiêu ?</label>
            <input type="text" name="debit_limit" class="form-control" value="">
          </div>
          
          <div class="service-mode__block" >
            <div class="form-group" style="display: none;">
            <label>Xác nhận SMS</label>
              <select class="form-control" name="tel_type">
<option value="1" selected>Không chấp nhận</option>
...
<option value="1"';
  ...
  $return .= '>Không chấp nhận</option>
<option value="2"';
  if ($user["tel_type"] == 2):
    $return .= 'selected';
  endif;
  $return .= '>Chấp nhận</option>
                </select>
            </div>
          </div>
...
<script>
          ...
          ';
  echo json_encode(["content" => $return, "title" => "Chỉnh sửa Thành viên"]);
...
elseif ($action == "pass_user"):
  ...
  echo json_encode(["content" => $return, "title" => "Sửa mật khẩu"]);
elseif ($action == "alert_user"):
  $return = '<form class="form" action="' . site_url("admin/clients/alert") . '" method="post" data-xhr="true">
        <div class="modal-body">
          ...
          <div class="service-mode__block">
            <div class="form-group">
            <label>Thành viên được thông báo</label>
              <select class="form-control" id="user_type" name="user_type">
<option value="all">Tấ cả Thành viên</option>
<option value="secret">Thành viên cụ thể</option>
                </select>
            </div>
          </div>
...
<option value="sms">SMS</option>
        ...
        <script type="text/javascript">
          ...
          <div class="modal-footer">
            <button type="submit" class="btn btn-primary">Thông báo cho người dùng</button>
            <button type="button" class="btn btn-danger" data-dismiss="modal">Hủy</button>
          </div>
          </form>
...
elseif ($action == "new_service"):
  ...
  foreach ($languages as $language):
    if ($language["default_language"]):
      $return .= '
     <div class="form-group">
              <label class="form-group__service-name">Icon dịch vụ </label>
               <input type="text" class="form-control" id="services_icon" name="services_icon" placeholder="Ví dụ: fa-cogs">
            </div>
      <div class="form-group">
              <label class="form-group__service-name">Tên dịch vụ <span class="badge">' . $language["language_name"] . '</span> ' . $translationList . ' </label>
      ...
      $return .= '<div class="form-group">
              <label class="form-group__service-name">Tên dịch vụ <span class="badge">' . $language["language_name"] . '</span> </label>
              <input type="text" class="form-control" name="name[' . $language["language_code"] . ']" value="' . $multiName[$language["language_code"]] . '">
            </div>';
    endif;
  endforeach;
...
// Đảm bảo bạn đã thêm FontAwesome vào dự án của mình
$return .= '
...
<option value="0">Please select a category..</option>';
  ...
  <option value="3">Bình luận đặc biệt</option>
  <option value="4">Gói bình luận</option>
                  </select>
              </div>
            </div>
            <div class="service-mode__block">
  ...
  <option value="2">Tự động API</option>
                  </select>
              </div>
            </div>
  ...
  $return .= '</select>
              ...
              <div class="service-mode__block"  style="display: none">
                <div class="form-group">
                <label>Giá trên giá mua</label>
                  <select class="form-control" name="saleprice_cal" id="saleprice_cal>
<option value="normal">No</option>
<option value="percent">Thêm % vào giá mua của bạn </option>
<option value="amount">Thêm số tiền vào giá mua của bạn </option>
                  </select>
                </div>
              </div>
              <div class="form-group" style="display: none">
                <label class="form-group__service-name">Giá tiền</label>
          ...
          <div class="service-mode__wrapper">
              <div class="row">
                <div class="col-md-6 service-mode__block ">
                  <div class="form-group">
                  <label>Kiểm tra quyền riêng tư của hồ sơ Instagram?</label>
...
<select class="form-control" name="start_count">
      <option value="none">Đừng rút lui</option>
      <option value="instagram_follower">Số lượng người theo dõi trên Instagram</option>
      <option value="instagram_photo">Lượt thích ảnh trên Instagram</option>
  </select>
                  </div>
                </div>
              </div>
              <div class="row">
                <div class="col-md-6 service-mode__block ">
                  <div class="form-group">
                  <label>Nhập lệnh thứ 2 vào cùng một liên kết?</label>
<select class="form-control" name="instagram_second">
      ...
      <option value="1">Không</option>
          ...
          <div class="form-group">
            <label class="form-group__service-name">Giá dịch vụ (1000) <span class="badge badge-secondary">' . $settings["site_base_currency"] . " (" . get_currency_symbol_by_code($settings["site_base_currency"]) . ')</span></label>
            <input type="text" class="form-control" name="price" value="">
          </div>

          <div class="row">
            <div class="col-md-6 form-group">
              <label class="form-group__service-name">Tối thiểu</label>
              <input type="text" class="form-control" name="min" value="">
            </div>

            <div class="col-md-6 form-group">
              <label class="form-group__service-name">Tối đa</label>
...
<div class="service-mode__block">
            <div class="form-group">
            <label>Nút bảo hành</label>
              <select class="form-control" name="show_refill">
                  <option value="false">Mở</option>
                  <option value="true">Tắt</option>
              </select>
            </div>
          </div>
<div class="row" id="refill">
            <div class="col-md-6 form-group">
              <label class="form-group__service-name">Ngày bảo hành</label>
              <input type="text" class="form-control" name="refill_days" value="">
            </div>

            <div class="col-md-6 form-group">
              <label class="form-group__service-name">Màn hình Bảo hành (in hours)</label>
...
<div class="service-mode__block">
          ...
          <div class="service-mode__block">
            <div class="form-group">
            <label>Tốc độ dịch vụ</label>
              <select class="form-control" name="speed">
                  <option value="1">Rất chậm</option>
                  <option value="2">Tốc độ chậm</option>
                  <option value="3">Bình thường</option>
                  <option value="4">Rất nhanh</option>
              </select>
            </div>
          </div>
  ...
  $return .= 'script.js"></script>
          ...
          ';
  echo json_encode(["content" => $return, "title" => "Thêm dịch vụ mới"]);
...
<input type="text" class="form-control" name="name[' . $language["language_code"] . ']" value="' . $multiName[$language["language_code"]] . '">
    ...
    $return .= '<div class="service-mode__block">
                ...
                <label>Danh mục</label>
                  ...
                  <select class="form-control" name="category">
    <option value="0">Vui lòng chọn một danh mục..</option>';
    foreach ($categories as $category):
    ...
    $return .= '>Tự động thích Instagram - Không giới hạn</option>
    <option value="12"';
    if ($serviceInfo["service_package"] == 12):
      $return .= 'selected';
    endif;
    $return .= '>Theo dõi tự động Instagram - Không giới hạn</option>
    <option value="14"';
    if ($serviceInfo["service_package"] == 14):
      $return .= 'selected';
    endif;
    $return .= '>Tự động thích Instagram - Có thời gian</option>
    <option value="15"';
    if ($serviceInfo["service_package"] == 15):
      $return .= 'selected';
    endif;
    $return .= '>Instagram Auto Watch - Có thời gian</option>
</select>
                </div>
              </div>
...
<input type="text" class="form-control" name="max" value="' . $serviceInfo["service_max"] . '">
              ...
              <div id="limited">
                <div class="form-group">
                  <label class="form-group__service-name">Giá dịch vụ/label>
                ...
                <div class="row">
                  <div class="col-md-6 form-group">
<label class="form-group__service-name">Số lượng hàng hóa</label>
<input type="text" class="form-control" name="autopost" value="' . $serviceInfo["service_autopost"] . '">
                  </div>

                  <div class="col-md-6 form-group">
<label class="form-group__service-name">Số lượng đặt hàng</label>
<input type="text" class="form-control" name="limited_min" value="' . $serviceInfo["service_min"] . '">
                  </div>
                </div>
                <div class="form-group">
                  <label class="form-group__service-name">Thời gian gói hàng <small> (days)</small></label>
  ...
  <option value="1"';
    ...
    $return .= '>Rất chậm</option>
  <option value="2"';
    if ($serviceInfo["service_speed"] == 2):
      $return .= 'selected';
    endif;
    $return .= '>Tốc độc chậm</option>
  <option value="3"';
    if ($serviceInfo["service_speed"] == 3):
      $return .= 'selected';
    endif;
    $return .= '>Bình thường</option>
  <option value="4"';
    if ($serviceInfo["service_speed"] == 4):
      $return .= 'selected';
    endif;
    $return .= '>Tốc độ nhanh</option>
                  </select>
                </div>
              </div>
              ...
              <div class="modal-footer">
                <button type="submit" class="btn btn-primary">Cập nhật thông tin đăng ký</button>
                <button type="button" class="btn btn-danger" data-dismiss="modal">Hủy</button>
              </div>
              </form>
<script>
...
// Giả sử bạn đã có kết nối PDO được lưu trong biến $conn
$serviceId = $_POST["id"]; // Hoặc giá trị tương ứng mà bạn muốn lấy dịch vụ

$serviceQuery = $conn->prepare("SELECT services_icon FROM services WHERE service_id = :id");
$serviceQuery->execute(array("id" => $serviceId));
$services = $serviceQuery->fetch(PDO::FETCH_ASSOC);
$return .= '<div class="form-group">
    <label class="form-group__service-name">Icon dịch vụ </label>
    <div class="input-icon-container">
        <input type="text" class="form-control" name="services_icon" id="services_icon" value="' . htmlspecialchars($services["services_icon"]) . '">
        <div class="icon-preview">' . $services["services_icon"] . '</div>
    </div>
</div>';
...
<select class="form-control" name="package">
    ...
    $return .= '>Bình luận đặc biệt</option>
      <option value="4"';
    if ($serviceInfo["service_package"] == 4):
      $return .= 'selected';
    endif;
    $return .= '>Gói bình luận</option>
  </select>
                  </div>
                </div>
                <div class="service-mode__block">
...
<label>Giá trên giá mua</label>
  <select class="form-control" name="saleprice_cal" id="saleprice_cal>
    <option value="normal">Không</option>
    <option value="percent">Thêm % vào giá mua của bạn</option>
    <option value="amount">Thêm số tiền vào giá mua của bạn </option>
  </select>
</div>
...
<input type="text" class="form-control" name="saleprice" value="">
                  ...
                  <div class="service-mode__block">
<div class="form-group">
<label>Nhỏ giọt</label>
  <select class="form-control" name="dripfeed">
    <option value="1"';
    if ($serviceInfo["service_dripfeed"] == 1):
      $return .= 'selected';
    endif;
    $return .= '>Không hoạt động</option>
    <option value="2"';
    if ($serviceInfo["service_dripfeed"] == 2):
      $return .= 'selected';
    endif;
    $return .= '>Hoạt động</option>
  </select>
</div>
                  </div>
                </div>
...
<div class="col-md-6 service-mode__block ">
  ...
  <label>Số bắt đầu</label>
    ...
    $return .= '>Số bắt đầu</option>
          <option value="instagram_follower"';
    if ($serviceInfo["start_count"] == "instagram_follower"):
      $return .= 'selected';
    endif;
    $return .= '>Số lượng người theo dõi trên Instagram</option>
...
<div class="col-md-6 form-group" id="refill">
<label class="form-group__service-name">Thời gian Bảo hành (in hours)</label>
...
<label class="form-group__service-name">Tràn đầy (%)</label>
<input type="text" class="form-control" name="service_overflow" value="' . $serviceInfo["service_overflow"] . '">
</div>

<div class="form-group" id="refill"> 

<label class="form-group__service-name">Vô hiệu hóa đồng bộ</label>
...
<label>Link đơn hàng</label>
  ...
  <option value="4"';
    ...
    $return .= '>Nhanh</option>
              ...
              <div class="modal-footer">
                <button type="submit" class="btn btn-primary">Cập nhật thông tin dịch vụ</button>
                <button type="button" class="btn btn-danger" data-dismiss="modal">Hủy</button>
              </div>
        </form>
<script>
...
if( type == "11" || type == "12" ){
  ...
  $("#limited").show();
              ...
              ';
    echo json_encode(["content" => $return, "title" => "Chỉnh sửa Dịch vụ (ID: " . $serviceInfo["service_id"] . ")"]);
  endif;
  elseif ($action == "edit_service_name"):
    $id = $_POST["id"];
    $smmapi = new SMMApi();
    $serviceInfo = $conn->prepare("SELECT service_id,service_name,name_lang,services_icon FROM services WHERE service_id=:id ");
    $serviceInfo->execute(array("id" => $id));
    $serviceInfo = $serviceInfo->fetch(PDO::FETCH_ASSOC);
...
<div class="modal-body">';
  ...
  foreach ($languages as $language):
    if ($language["default_language"]):
      $return .= '
      <div class="form-group">
<label class="form-group__service-name">Mô tả dịch vụ <span class="badge">' . $language["language_name"] . '</span> ' . $translationList . ' </label>
<textarea rows="10" class="form-control" name="service_description[' . $language["language_code"] . ']" >' . rtrim(str_replace(["<br>", "<br/>", "<br />"], "\n", $multiDesc[$language["language_code"]])) . '</textarea>
      ...
      $return .= '
      <div class="form-group">
      <label class="form-group__service-name">Mô tả dịch vụ <span class="badge">' . $language["language_name"] . '</span></label>
...
<div class="modal-footer">
<button type="submit" class="btn btn-primary">Cập nhật mô tả</button>
<button type="button" class="btn btn-danger" data-dismiss="modal">Hủy</button>
</div>
</form>
<script>
...
';
  echo json_encode(["content" => $return, "title" => "Chỉnh sửa mô tả (ID: " . $serviceInfo["service_id"] . ")"]);
elseif ($action == "edit_time"):
  ...
  foreach ($languages as $language):
    if ($language["default_language"]):
      $return .= '<div class="form-group">
<label class="form-group__service-name">Giải thích <span class="badge">' . $language["language_name"] . '</span> ' . $translationList . ' </label>
<textarea class="form-control" rows="5" name="description[' . $language["language_code"] . ']">' . $multiDesc[$language["language_code"]] . '</textarea>
      ...
      $return .= '<div class="form-group">
<label class="form-group__service-name">Giải thích <span class="badge">' . $language["language_name"] . '</span> </label>
<textarea class="form-control" rows="5"  name="description[' . $language["language_code"] . ']">' . $multiDesc[$language["language_code"]] . '</textarea>
                  </div>';
    endif;
  endforeach;
  ...
  $return .= '
              ...
              <div class="modal-footer">
                <button type="submit" class="btn btn-primary">Thời gian cập nhật</button>
                <button type="button" class="btn btn-danger" data-dismiss="modal">Hủy</button>
              </div>
              </form>
              <script type="text/javascript">
              ...
              ';
  echo json_encode(["content" => $return, "title" => "Chỉnh sửa Thời gian trung bình (ID: " . $serviceInfo["service_id"] . ")"]);
...
<option value="11">Instagram Auto Likes - Không giới hạn</option>
<option value="12">Instagram Auto Tracking - Không giới hạn</option>
<option value="14">Instagram Auto Likes - Đã hẹn giờ</option>
<option value="15">Instagram Auto Watch - Đã hẹn giờ</option>
                </select>
            </div>
          </div>
  ...
  $return .= '</select>
          ...
          <div class="service-mode__block">
            ...
            <label>Tốc độ dịch vụ</label>
              <select class="form-control" name="speed">
                  <option value="1">Rất chậm</option>
                  <option value="2">Chậm Slow</option>
                  <option value="3">Bình thường</option>
          ...
          <div class="modal-footer">
            <button type="submit" class="btn btn-primary">Thêm dịch vụ API</button>
            <button type="button" class="btn btn-danger" data-dismiss="modal">Hủy</button>
          </div>
          </form>
          <script type="text/javascript">
          ...
          ';
  echo json_encode(["content" => $return, "title" => "Thêm dịch vụ API"]);


elseif ($action == "new_category"):
  $return = '<form class="form" action="' . site_url('admin/services/new-category') . '" method="post" data-xhr="true">' . "\r\n\r\n" . '<div class="modal-body">' . "\r\n" . ' <div class="form-group">' . "\r\n" . '  <label class="form-group__service-name">Tên danh mục</label>' . "\r\n" . '  <input type="text" class="form-control" name="name" value="">' . "\r\n" . ' </div>' . "\r\n\r\n\r\n" . ' <div class="form-group">' . "\r\n" . '  <label class="form-group__service-refill">Chức vụ  <div class="tooltip5">  <span class="fas fa-info-circle"></span><span class="tooltiptext5">Vị trí của một danh mục sau khi thêm nó</span></div> </label>' . "\r\n" . ' <select name="position" class="form-control"><option value="top">Top</option><option value="bottom">Ở dưới</option></select> ' . "\r\n" . ' </div>' . "\r\n\r\n\r\n" . ' <div class="service-mode__block">' . "\r\n" . '  <div class="form-group">' . "\r\n" . '  <label>Ẩn danh mục</label>' . "\r\n" . '<select class="form-control" name="secret">' . "\r\n" . '  <option value="2">Không</option>' . "\r\n" . '  <option value="1">Có</option>' . "\r\n" . '</select>' . "\r\n" . '  </div>' . "\r\n" . ' </div>' . "\r\n\r\n\r\n" . '</div>' . "\r\n\r\n" . ' <div class="modal-footer">' . "\r\n" . '  <button type="submit" class="btn btn-primary">Tạo danh mục</button>' . "\r\n" . '  <button type="button" class="btn btn-danger" data-dismiss="modal">Hủy</button>' . "\r\n" . ' </div>' . "\r\n" . ' </form>';
...
elseif ($action == "edit_category"):
  ...
  foreach ($languages as $language):
    if ($language["default_language"]):
      $return .= '
        <div class="form-group">
<label class="form-group__service-name">Tên danh mục <span class="badge">' . $language["language_name"] . '</span> ' . $translationList . ' </label>
<input type="text" class="form-control" name="category_name[' . $language["language_code"] . ']" value="' . $multiName[$language["language_code"]] . '">
      ...
      $return .= '<div class="form-group">
<label class="form-group__service-name">Tên danh mục <span class="badge">' . $language["language_name"] . '</span> </label>
<input type="text" class="form-control" name="category_name[' . $language["language_code"] . ']" value="' . $multiName[$language["language_code"]] . '">
                </div>';
    endif;
  endforeach;
...
<li  class="list-group-item active enable-icon-picker">Icon có sẵn</li>
<li  class="list-group-item enable-image-picker">Icon tự chọn</li>
</ul>
...
<div style="display:none;" class="picker imagepicker-div load-images"></div>';
  ...
  $return .= '<hr color="#2176FF"><button type="button" id="update-category" class="btn btn-primary">Cập nhật Danh mục</button>' . "\r\n" . '  <button type="button" class="btn btn-danger" data-dismiss="modal">Hủy</button>';
  $return .= '<script>
...
$(".modal .close").trigger("click");
  ...
  echo json_encode(['content' => $return, 'title' => 'Chỉnh sửa Danh mục <span class="badge badge-primary">' . $id . '</span>']);
...
elseif ($action == "download_category_icon_images"):
  ...
  "pin": "https://i.postimg.cc/05G3cRfF/pin.png"
    ...
    $return .= '<label>Chọn Icon Danh mục</label>
...
</form><div class="form-group"><button type="button" id="upload_an_image" class="btn btn-primary">Tải lên một hình ảnh</button></div>
<script>
...
elseif ($action == "import_services"):
  ...
  $return .= '</select>
              ...
              <div class="form-group">
              <label>Chọn danh mục để thêm dịch vụ</label>
  ...
  $return .= '</select>
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-danger" data-dismiss="modal">Hủy</button>
            <button type="button" class="btn btn-primary" id="nextStep" data-step="first">Bước tiếp theo</button>
            <button type="submit" class="btn btn-primary" id="submitStep">Thêm dịch vụ</button>
          </div>
...
$.toast({
    heading: "Không thành công",
    text: "Vui lòng chọn nhà cung cấp dịch vụ",
    icon: "error",
    loader: true,
    loaderBg: "#9EC600"
});
...
var array     = [];
 ...
 if( count ){
   ...
   $.toast({
       heading: "Không thành công",
       text: "Vui lòng chọn ít nhất 1 dịch vụ bạn muốn thêm",
       icon: "error",
       loader: true,
       loaderBg: "#9EC600"
   });
...
<div class="services-import__list-wrap">
   <div class="services-import__scroll-wrap">
					   <label class="btn btn-primary"> <input id="checkk" type="checkbox"> Chọn tất cả</label>';
      foreach ($grouped as $category):
        $category_id++;
        echo '
      <span>
...
elseif ($action == "import_services_last"):
  ...
  <div class="services-import__placeholder-title">Chọn tiền tệ</div><br>
					  <select id="raise-currency" name="currency">
        <option value="" disabled selected>Chọn Tiền tệ của Nhà cung cấp</option>
        ...
        <option value="1">USD</option>
                   ...
                   <div class="services-import__step3-field">
  <div class="services-import__placeholder-title">Phần trăm (%)</div>
  <input type="number" placeholder="0" id="raise-percent" name="percent" value="">
                   </div>
				   
                   <div class="services-import__step3-actions"><span class="btn btn-danger">Thiết lập lại tính toán</span></div>
                </div>
                <div class="services-import__list-wrap services-import__list-active">
                   <div class="services-import__scroll-wrap">';
  $category_id = 0;
 ...
 if( count ){
   ...
   $.toast({
       ...
       loaderBg: "#9EC600"
          ...
          ';
  echo json_encode(["content" => $return, "title" => "Rút dịch vụ khỏi nhà cung cấp"]);
elseif ($action == "import_services_list"):
  $provider_id = $_POST["provider"];
  $category_id2 = $_POST["category"];
  $smmapi = new SMMApi();
...
elseif ($action == "new_ticket"):
  $return = '<form class="form" action="' . site_url("admin/tickets/new") . '" method="post" data-xhr="true">
        <div class="modal-body">
          ...
          <div class="form-group">
            <label class="form-group__service-name">Thành viên</label>
            <input type="text" class="form-control" name="username" value="">
          </div>

          <div class="form-group">
            <label class="form-group__service-name">Đề tài</label>
            <input type="text" class="form-control" name="subject" value="">
          </div>

          <div class="form-group">
            <label class="form-group__service-name">Tin nhắn</label>
          ...
          <div class="modal-footer">
            <button type="submit" class="btn btn-primary">Tạo yêu cầu mới</button>
            <button type="button" class="btn btn-danger" data-dismiss="modal">Hủy</button>
          </div>
          </form>';
  echo json_encode(["content" => $return, "title" => "Yêu cầu hỗ trợ mới"]);
elseif ($action == "yeni_kupon"):
  $return = '<form class="form" action="' . site_url("admin/kuponlar/new") . '" method="post" data-xhr="true">
        <div class="modal-body">

          <div class="form-group">
            <label class="form-group__service-name">Mã giảm giá</label>
            <input type="text" class="form-control" name="kuponadi" value="">
          </div>

          <div class="form-group">
            <label class="form-group__service-name">Cái</label>
            <input type="text" class="form-control" name="adet" value="">
          </div>

          <div class="form-group">
            <label class="form-group__service-name">Số tiền</label>
          ...
          <div class="modal-footer">
            <button type="submit" class="btn btn-primary">Tạo phiếu giảm giá mới</button>
            <button type="button" class="btn btn-danger" data-dismiss="modal">Hủy</button>
          </div>
          </form>
          ';
  echo json_encode(["content" => $return, "title" => "Tạo phiếu giảm giá mới"]);
...
elseif ($action == "edit_integration" && $_POST["id"] == "whatsapp"):
  ...
  $return .= '>Tất cả</option>' . "\r\n" . ' <option value="2"';

  if (extra['visibility'] == 2) {
    $return .= 'selected';
  }

  $return .= '>Bên ngoài</option>' . "\r\n" . '  <option value="1"';

  if ($extra['visibility'] == 1) {
    $return .= 'selected';
  }

  $return .= '>Nội bộ</option>' . "\r\n" . '</select>' . "\r\n" . '  </div>' . "\r\n" . ' </div>' . "\r\n\r\n" . ' </div>' . "\r\n\r\n" . ' </div>' . "\r\n\r\n\r\n" . '</div>' . "\r\n\r\n" . ' <div class="modal-footer">' . "\r\n" . '  <button type="submit" class="btn btn-primary">Cập nhật</button>' . "\r\n" . '  <button type="button" class="btn btn-danger" data-dismiss="modal">Hủy</button>' . "\r\n" . ' </div>' . "\r\n" . ' </form>';
...
elseif ($action == "edit_paymentmethod" && $_POST["id"] == "phonepe"):
  ...
  $return .= '>Nội bộ</option>' . "\r\n" . '</select>' . "\r\n" . '  </div>' . "\r\n" . ' </div>' . "\r\n\r\n" . ' <div class="form-group">' . "\r\n" . '  <label class="form-group__service-name">Tên hiển thị</label>' . "\r\n" . '  <input type="text" class="form-control" name="name" value="' . $extra['name'] . '">' . "\r\n" . ' </div>' . "\r\n\r\n" . ' <div class="form-group">' . "\r\n" . '  <label class="form-group__service-name">Thanh toán tối thiểu</label>' . "\r\n" . '  <input type="text" class="form-control" name="min" value="' . $extra['min'] . '">' . "\r\n" . ' </div>' . "\r\n\r\n" . ' <div class="form-group">' . "\r\n" . '  <label class="form-group__service-name">Thanh toán tối đa</label>' . "\r\n" . '  <input type="text" class="form-control" name="max" value="' . $extra['max'] . '">' . "\r\n" . ' </div>' . "\r\n\r\n" . ' <hr>' . "\r\n" . '  <p class="card-description">' . "\r\n" . '<ul>' . "\r\n" . '<li>' . "\r\n" . ' Địa chỉ gọi lại API: <code>';
  $return .= site_url('payment/' . $method['method_get']);
  $return .= '</code>' . "\r\n" . '</li>' . "\r\n" . '</ul>' . "\r\n" . '  </p>' . "\r\n" . ' <hr>' . "\r\n\r\n  <div class=\"form-group\"><label class=\"form-group__service-name\">Liên kết hình ảnh QR Phonepe</label><input type=\"text\" class=\"form-control\" name=\"phonepe_qr_link\" value=\"" . $extra["phonepe_qr_link"] . "\"/></div>" . '<div class="form-group">' . "\r\n" . '  <label class="form-group__service-name">Email</label>' . "\r\n" . '  <input id="gmail" type="text" class="form-control" name="email" value="' . $extra['email'] . '">' . "\r\n" . ' </div>' . '

<div style="padding:20px;box-shadow: rgba(0, 0, 0, 0.15) 1.95px 1.95px 2.6px;" class="form-group">
<p style="font-weight:bold;">Để đọc Giao dịch PhonePe từ Gmail, chúng tôi cần quyền truy cập vào Tài khoản Gmail của bạn. Nhấp vào nút bên dưới để cho phép truy cập. Bạn có thể thấy một thông báo <span class="text-primary">"Google hasn\'t verified this app."</span> Click <span class="text-primary">"Advanced"</span> and click <span class="text-primary">"Tiếp tục".</span></p>

<div id="gmail_access"></div>
<br>
<p style="font-weight:bold;">Sau đó, bạn sẽ nhận được một <span class="text-primary">"Phím truy cập".</span> Sao chép khóa truy cập đó và dán vào trường bên dưới.</p>
</div>
<script>
...
$("#gmail_access").html(\'<a class="btn btn-primary" target="_blank" href="https://mails.dukesmm.com/oauth?domain=' . $_SERVER["HTTP_HOST"] . '&email=\'+email+\'">Allow Access for \'+email+\' </a>\');
  ...
  $("#gmail_access").html(\'<button type="button" class="btn btn-danger" disabled>Email bạn nhập không phải là Địa chỉ Gmail</button>\');
}
}
$(document).ready(function(){
...
<input type="checkbox" class="access" name="exportcolumn[telephone]" checked value="1"> Phone number
                  ...
                  <label class="checkbox-inline">
<input type="checkbox" class="access" name="exportcolumn[balance]" checked value="1"> Số dư
                  </label>
                  <label class="checkbox-inline">
...
elseif ($action == "reffered_users"):
      ...
      <div class="modal-body">
        ...
        <div class="service-mode__block">
          <div class="form-group">
            ...
            <th>Thành viên</th><th>Số dư</th><th>Spent</th><th>Hoạt động </th>
            </thead>';
  foreach ($clients as $client):
    ...
    $return .= '<tr>
                ...
                <td>' . $client['spent'] . '</td>
                <td><a href="admin/referrals?ref_code=' . $ref_code . '&remove=' . $client['client_id'] . '">Xóa</a></td>
              </tr>';
  endforeach;
...
elseif ($action == "add_admin" && $admin["admin_type"] == 3):
  ...
  $return .= '</optgroup> </select>
                ...
                ';
  echo json_encode(["content" => $return, "title" => "Thêm Admin"]);
...
elseif ($action == "edit_currency"):
  ...
  $return = '<form class="form" action="' . site_url("admin/settings/currency/edit/" . $id) . '" method="post" data-xhr="true">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            <label class="form-group__service-name">Tên tiền tệ</label>
            <input type="text" class="form-control" name="name" value="' . $provider["name"] . '">
          </div>
          <div class="form-group">
            <label class="form-group__service-name">Ký hiệu tiền tệ</label>
            <input type="text" class="form-control" name="symbol" value="' . $provider["symbol"] . '">
          </div>
<div class="form-group">
            <label class="form-group__service-name">Tỷ giá hối đoái</label>
...
<div class="service-mode__block">
                <div class="form-group">
                <label>Trạng thái tiền tệ</label>
  ...
  echo json_encode(["content" => $return, "title" => "Chỉnh sửa tiền tệ (" . $provider["name"] . ") "]);
