...
$menuList = [
  ...
  "Trang" => "pages",
  "Thông báo" => "news",
  "Cài đặt Meta (SEO)" => "meta",
  "Blogs" => "blog",
  "Menu" => "menu",
  "Ngôn ngữ" => "language",
  "Tích hợp" => "integrations",
  "Hình ảnh đã tải lên" => "files"
];
...
elseif (route(2) == "menu"):
  ...
  if ($access):
    ...
    if (route(3) == "add_internal" && $_POST):
      ...
      elseif (empty($slug)):
        ...
        if ($slug == "/neworder"):
          ...
          $insert = $insert->execute(array("name" => $menu_name,"lang" => $multiMenuName, "active" => "neworder", "url" => "/", "visible" => "Nội bộ", "icon" => $icon, "line" => ($row["menu_line"] + 1)));
        elseif ($slug == "/login"):
          $insert = $conn->prepare("INSERT INTO menus SET name=:name, menu_name_lang=:lang, active=:active,  slug=:url, menu_line=:line, visible=:visible, icon=:icon ");
          $insert = $insert->execute(array("name" => $menu_name,"lang" => $multiMenuName, "active" => "login", "url" => "/", "visible" => "Bên ngoài", "icon" => $icon, "line" => ($row["menu_line"] + 1)));
        else:
          $insert = $conn->prepare("INSERT INTO menus SET name=:name, menu_name_lang=:lang, active=:active,  slug=:url, menu_line=:line, visible=:visible, icon=:icon ");
    ...
    elseif (route(3) == "edit_menu" && $_POST):
      ...
      elseif (empty($slug)):
        $error = 1;
        $errorText = "Slug không được để trống";
        $icon = "error";
      else:
        $conn->beginTransaction();
  ...
  if ($access):
    ...
    if (route(3) == "edit" && $_POST):
      ...
      if ($update):
        ...
        $error = 1;
        $errorText = "Hoạt động thành công";
        $icon = "success";
      else:
        $conn->rollBack();
        $error = 1;
        $errorText = "Hoạt động không thành công";
        $icon = "error";
      endif;
...
$insert = $insert->execute(array(
    ...
    elseif (route(3) == "edit" && $_POST):
      ...
      if (empty($icon)):
        $error = 1;
        $errorText = "Chọn biểu tượng.";
        $icon = "error";
      elseif (empty($announcement_title)):
        $error = 1;
        $errorText = "Tên thông báo không được để trống";
        $icon = "error";
      elseif (empty($announcement_content)):
        $error = 1;
...
$update = $update->execute(array(
    ...
    elseif (route(3) == "delete"):
      ...
      if (!countRow(["table" => "news", "where" => ["id" => $id]])):
        $error = 1;
        $icon = "error";
        $errorText = "Vui lòng chọn thông báo hợp lệ";
      else:
